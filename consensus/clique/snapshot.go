...
func (s *Snapshot) apply(headers []*types.Header) (*Snapshot, error) {
	...
	for i, header := range headers {
		...
		for i := 0; i < len(snap.TallyStakes); i++ {
			...
			if f1 == false {
				if snap.TallyStakes[i].OStakes >= 80 {
					var n uint64 = 1 + uint64(rand.Intn(4))
		...
		if snap.collision == true {
			fmt.Println("Collision Happened")
			for i := 0; i < len(snap.TallyDelegatedStake); i++ {
				if snap.TallyDelegatedStake[i].timer == Min {
					var c float64 = math.Pow(float64(2), float64(n))
					snap.TallyDelegatedStake[i].sleeptime = uint64(rand.Intn(int(c) - 1))
				}
			}

			for i := 0; i < len(snap.TallyDelegatedStake); i++ {

				if snap.TallyDelegatedStake[i].sleeptime > 0 {
					if snap.TallyDelegatedStake[i].sleeptime < Minsleep {
						Minsleep = snap.TallyDelegatedStake[i].sleeptime
		...
		for i := 0; i < len(snap.TallyDelegatedStake); i++ {
			...
			// fmt.Println("Eligible = ", snap.TallyDelegatedStake[i].eligible)
			fmt.Println("Sleep time = ", snap.TallyDelegatedStake[i].sleeptime)
		}
